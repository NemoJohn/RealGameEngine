...
{
    float4 noise = tex2D(DistortionSampler, float2(input.Tex0.x + time_0_X/10, input.Tex0.y+time_0_X/10));
    float scale = pow(noise.g, 8);
    float2 newUV = input.Tex0 + float2(scale, scale);
    float4 baseColor = tex2D(FireSampler, newUV);
